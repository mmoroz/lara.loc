<x-app-layout>

    <div class="pt-2 p-4 text-gray-900">
        <h1 class="py-4">Добавить регион</h1>

        <form method="POST" action="{{ route('admin.regions.update', $region) }}">
            @csrf
            @method('PUT')

            <div class="flex flex-wrap mb-4 w-full">

                <div class="w-1/2 pr-4">
                    <x-input-label for="name" :value="__('Наименование')" />
                    <x-text-input value="{{ old('name', $region->name) }}" id="name" name="name" type="text" class="mt-1 block w-full" />
                    @if ($errors->has('name'))
                        <span class="text-red-600"><strong>{{ $errors->first('name') }}</strong></span>
                    @endif
                </div>

                <div class="w-1/2">
                    <x-input-label for="slug" :value="__('Slug')" />
                    <x-text-input value="{{ old('slug', $region->slug) }}" id="slug" name="slug" type="text" class="mt-1 block w-full" />
                    @if ($errors->has('slug'))
                        <span class="text-red-600"><strong>{{ $errors->first('slug') }}</strong></span>
                    @endif
                </div>

            </div>

            <x-primary-button>{{ __('Сохранить') }}</x-primary-button>


        </form>
    </div>

</x-app-layout>

